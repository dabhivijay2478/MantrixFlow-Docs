...
Use the **dbt Layer** tab in the **Destination** panel.

Reference staged source data with the dbt source helper:
...
The live Normalisation rules are Rename and Cast. They run before the dbt Layer.
...
- dbt model output contains a column missing from the destination table
- Incremental sync is missing a replication key
- credentials changed after the connection was saved

## Is staging data permanent?

No. Runs stage data in a temporary DuckDB file. The runner extracts checkpoint state and deletes the DuckDB file during cleanup.
