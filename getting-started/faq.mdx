...
Asana, Notion, HubSpot, and Stripe are available as sources. Asana exposes
eight supported streams, Notion discovers each shared data source dynamically,
Stripe exposes 34, and HubSpot exposes ten. MySQL, Airtable, Asana, and Notion
are runtime-capability gated, so the connection picker shows them only when both
