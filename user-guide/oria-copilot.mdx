...
Oria automatically chooses the appropriate read-only capability based on your
question and context. You do not need to select or manage an internal agent.
...
workspace access. Your deployment administrator must also enable Oria for the
environment.

### Oria is temporarily unavailable

Refresh the page once. If the problem continues, ask an administrator to check
the configured AI provider, model, server feature flag, and provider
credentials. Provider credentials must remain server-side.
