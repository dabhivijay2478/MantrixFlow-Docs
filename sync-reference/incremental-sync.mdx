...
- saved dbt model output that matches the destination columns
...
After a successful run, MantrixFlow extracts checkpoint state before deleting the temporary DuckDB file. The next Incremental run resumes from that stored state.
