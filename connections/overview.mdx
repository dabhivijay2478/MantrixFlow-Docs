...
description: Create and test database and SaaS connections supported by MantrixFlow, including the GitHub Beta source.
---

Connections are reusable encrypted credentials. PostgreSQL, MySQL, Airtable,
and ClickHouse are available as both sources and destinations. DuckDB is
visible as a Beta managed-artifact source and destination. HubSpot and
...
supported resources. GitHub is a Beta, source-only connector for read-only,
single-repository extraction with a Personal Access Token. Zendesk is a Beta, OAuth-only source with 13 Zendesk
Support resources. Strapi is a Beta, source-only connector for validated Strapi
5 Content API collections.
...
| Source | DuckDB | Beta; requires a pre-provisioned managed file reference and runtime support |
...
| Source | GitHub | Enabled Beta; Personal Access Token and one GitHub.com repository per connection |
| Source | Zendesk | Beta when OAuth and runtime capabilities are available |
| Source | Strapi | Beta when reported by runtime capabilities; Strapi 5 and Full Table only |
...
| Destination | DuckDB | Beta UI; durable artifact publication is not generally available yet |
...
`file_...` reference contract, read-only source behavior, and current
destination persistence limitations.
...
Full Table behavior, pagination safety, and Beta limitations.
...
4. Choose **PostgreSQL**, **MySQL**, **Airtable**, **ClickHouse**, **DuckDB**, **Asana**, **Linear**, **Notion**, **GitHub**, **Zendesk**, **Strapi**, **HubSpot**, or **Stripe**.
...
4. Choose **PostgreSQL**, **MySQL**, **Airtable**, **ClickHouse**, or **DuckDB**.
