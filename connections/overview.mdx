...
description: Create and test database and SaaS connections supported by MantrixFlow, including the MongoDB Beta source and destination.
---

Connections are reusable encrypted credentials. PostgreSQL, MySQL, Airtable,
and ClickHouse are available as both sources and destinations. MongoDB is an
explicitly enabled Beta source and destination using a complete encrypted
connection URI. DuckDB is visible as a Beta managed-artifact source and
destination. HubSpot and
...
| Source | MongoDB | Enabled Beta; dynamic collection discovery with Full Table and Incremental sync |
...
| Destination | MongoDB | Enabled Beta; key-required Upsert delivery |
