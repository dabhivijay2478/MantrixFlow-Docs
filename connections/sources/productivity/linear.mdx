...
Linear is available as an Available Now source connector. Arcyria reads eight
...
Arcyria never creates, edits, or deletes data in Linear.
...
3. Under **Personal API keys**, create a key for Arcyria.
...
<Note>
  Linear authentication is currently Personal API Key only in Arcyria.
  OAuth is not available for this connector yet.
</Note>

The key has the access of the Linear user who created it. Arcyria encrypts
...
5. Click **Test Connection**. Arcyria validates the key against the
...
Arcyria orders those resources by `updatedAt`, stores the latest cursor,
...
Linear list queries use cursor-based pagination. Arcyria follows
