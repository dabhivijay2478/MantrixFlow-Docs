...
Linear is available as a Beta source connector. MantrixFlow reads eight
...
If Linear is missing or disabled, refresh the page after the ELT runtime is
healthy. The connector is shown only when runtime capabilities report Linear
source support.
...
- Linear is Beta and source-only.
