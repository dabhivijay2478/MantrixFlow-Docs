...
Arcyria never writes to Notion.

## Supported resources

Notion separates a database container from the data sources inside that
database. Arcyria exposes each discovered data source as its own stable
...
2. Create an internal integration for the workspace that Arcyria will
   read.
3. Enable read-content access and copy the internal integration token.
4. Open each page or database that should be visible to Arcyria.
...
Treat the integration token like a password. Arcyria encrypts it at rest,
...
5. Click **Test Connection**. Arcyria validates the token against the
...
Arcyria then retrieves each data source directly to obtain authoritative
...
Arcyria fails the run instead of reporting partial data as complete. Split
...
than one, Arcyria asks for an explicit data-source selection rather than
guessing.
