...
Zendesk is Available Now as a source-only connector. Arcyria reads
...
You do not need to create or paste an API token. Arcyria uses Zendesk's
authorization-code OAuth flow and requests only the `read` scope. Zendesk
handles sign-in and approval on its own domain.

For a Arcyria production deployment serving multiple Zendesk accounts, the
...
8. Select **Allow**. Zendesk returns you to Arcyria and saves the
   connection.
...
Arcyria refreshes short-lived access tokens server-side. If authorization
...
to request records at or after the saved cursor. Arcyria preserves the
...
Arcyria follows Zendesk pagination links and honors `Retry-After` responses
...
| Zendesk shows a generic server-error page | Ask the Arcyria operator to verify the OAuth client identifier, confidential client kind, and exact registered callback URL. |
