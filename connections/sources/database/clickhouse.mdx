...
ClickHouse is available as a source connector. Arcyria connects over the
...
GRANT SELECT ON analytics.* TO arcyria_reader;
```
...
Arcyria stores checkpoint state only after a successful run. Failed runs
