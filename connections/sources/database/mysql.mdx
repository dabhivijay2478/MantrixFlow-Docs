...
MySQL is available as a source connector. Arcyria discovers tables visible
...
Do not enter `localhost` when Arcyria is running in a different container or
...
record changes. Arcyria advances the saved checkpoint only after a
successful run.
