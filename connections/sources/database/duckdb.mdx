...
`.duckdb` artifacts. Arcyria opens source artifacts read-only, discovers
...
<Warning>
  ...
  your Arcyria administrator before creating the connection.
</Warning>
...
filesystem path. Hosted Arcyria accepts only an authorized managed file
reference.
...
Arcyria opens DuckDB sources with read-only driver configuration for
