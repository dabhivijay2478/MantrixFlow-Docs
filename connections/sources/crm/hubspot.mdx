...
description: Connect HubSpot as a production source and deliver ten CRM streams to PostgreSQL with dlt.
---

HubSpot is available as a production, source-only connector. MantrixFlow uses
dlt to extract selected HubSpot data into isolated staging, applies your SQL/dbt
models, and delivers the result to existing PostgreSQL tables.

<Note>
  The supported path is **HubSpot → PostgreSQL**. HubSpot cannot be selected as
  a destination, and other destination types are not supported for this source.
</Note>
...
HubSpot exposes ten production streams:
...
1. Create or choose a PostgreSQL destination connection.
2. Create a pipeline using the saved HubSpot source.
3. Open the **Source** node and click **Discover schema**.
...
7. In the **Destination** node, map every selected stream to an existing
   PostgreSQL table and save an explicit SQL/dbt model.
8. Validate and preview the models, then run the pipeline.
9. Confirm the delivered rows in PostgreSQL and review the run in **Run
   History**.
...
Create or explicitly reconcile the destination schema and tables before
delivery. The pipeline runner does not create target tables during a run.
