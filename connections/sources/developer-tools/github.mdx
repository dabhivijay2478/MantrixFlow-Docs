...
GitHub is an enabled Beta, source-only connector. MantrixFlow reads selected
...
For the complete Beta resource set, use a classic Personal Access Token with
...
by resource. Classic PAT is the supported Beta contract for the GraphQL issue,
...
3. Select **GitHub**. The card is marked **Source only** and **Beta**.
...
The current Beta form accepts one repository per connection. GitHub Enterprise
...
- Live source-to-every-destination release validation remains in progress, so
  monitor Beta pipelines and verify destination row counts.
...
| Nested comments or labels stop at 100 | This is a current Beta limitation; split the use case or wait for nested pagination support. |
