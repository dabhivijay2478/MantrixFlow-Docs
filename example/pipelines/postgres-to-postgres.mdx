...
For a standalone library of SQL transform patterns (`{{ source }}`), see [Postgres-to-Postgres SQL examples](/example/pipelines/postgres-to-postgres-examples).
...
All examples below use `{{ source }}` as the table reference. MantrixFlow substitutes the actual source stream at run time.
