...
VALUES
  ...
  ('mysql-002', 'MySQL Beta', 'mysql-beta@example.test', UTC_TIMESTAMP(6)),
...
4. Preview rows and confirm `_airtable_record_id`, `External ID`, `Name`, and
   `Email` appear.
5. Keep **Full Table** mode and save.

### 3. Configure MySQL destination

1. Open the **Destination** node.
2. Choose the MySQL destination connection.
3. Set the final table to `analytics.airtable_customers`.
4. Publish this model, replacing the staging relation with the exact lowercase
...
5. Validate SQL and preview output.
6. Select `external_id` as the Upsert key.
7. Save the destination.

### 4. Run and verify

Start the pipeline and wait for **Succeeded**. Then run:
...
1. Discover the source database.
2. Select `source_app.customers`.
3. Preview the three test rows.
4. Use **Full Table** for the first run.
...
Validate and preview the output before configuring the Airtable fields.

### 4. Map Airtable destination fields

1. Choose the Airtable destination connection.
2. Click **Load bases** and select the destination base.
3. Select `Pipeline Sink`.
4. Map fields:
...
- the published SQL model uses the staging name shown by the builder;
...
- run history reports `failed rows = 0`.
...
| SQL relation does not exist | Copy the lowercase staging relation from the builder. |
...
6. Monitor run history for row-count changes, retries, and mapping failures.
